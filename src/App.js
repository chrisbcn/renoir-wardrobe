...
const handleApproveRecreatedItems = async (items, approvalType) => {
  for (const item of items) {
    const wardrobeItem = {
      ...
      analysis: { name: `${item.color} ${item.type}`, type: item.type }
    };
...
const App = () => {
  ...
  const ItemRecreationWorkflow = ({ detectedItems, originalImage, onApproveItem, onResetRecreation }) => {
    const [recreationStep, setRecreationStep] = useState('review');
    const [recreatedItems, setRecreatedItems] = useState([]);
    const [recreationProgress, setRecreationProgress] = useState(0);
      ...
      for (let i = 0; i < detectedItems.length; i++) {
        ...
        try {
          const response = await fetch('/api/recreate-item', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ item: item, originalImage: originalImage })
          });
          ...
          if (response.ok) {
            ...
            if (result.success) {
              ...
              recreated.push({ ...item, recreationFailed: true, error: result.error });
            }
          } else {
            recreated.push({ ...item, recreationFailed: true, error: 'API Error' });
          }
        } catch (error) {
          recreated.push({ ...item, recreationFailed: true, error: error.message });
        }
      }
      ...
      return (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <p className="text-gray-600 mb-4">
            We detected {detectedItems.length} clothing items. Recreate them as professional product photos?
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {detectedItems.slice(0, 4).map((item, index) => (
              <div key={index} className="text-center p-2 border rounded">
          ...
          <div className="flex gap-3">
            <button onClick={startRecreation} className="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700 transition-colors">
              Recreate as Product Photos
            </button>
            <button onClick={() => onApproveItem(detectedItems, 'skip_recreation')} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300 transition-colors">
              Skip & Add Current Items
            </button>
          </div>
        </div>
    ...
    if (recreationStep === 'recreating') {
      return (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">Creating Professional Product Photos...</h2>
          <div className="bg-gray-200 rounded-full h-3 mb-2">
            <div className="bg-purple-500 h-3 rounded-full transition-all" style={{ width: `${recreationProgress}%` }}></div>
          </div>
          <p className="text-sm text-gray-600">Processing item {Math.ceil((recreationProgress / 100) * detectedItems.length)} of {detectedItems.length}</p>
          <div className="flex items-center justify-center py-8">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600"></div>
            <span className="ml-3 text-gray-600">Generating professional product images...</span>
          </div>
    ...
    if (recreationStep === 'approve') {
      return (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold">Professional Product Photos</h2>
            <button onClick={onResetRecreation} className="text-sm text-gray-500 hover:text-gray-700">
              ← Back to Detection
            </button>
          </div>
          <div className="space-y-6">
            {recreatedItems.map((item, index) => (
              <div key={index} className="border rounded-lg p-4 bg-gray-50">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="bg-white p-3 rounded border">
                        ...
                        <p className="text-sm text-gray-600">{item.description}</p>
                        <div className="flex gap-2 mt-2">
                          <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">{item.color}</span>
                          <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">{Math.round(item.confidence * 100)}% confidence</span>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      {item.recreatedImage ? (
                        <div className="bg-white rounded border overflow-hidden">
                          <img src={item.recreatedImage} alt={item.productTitle} className="w-full h-48 object-cover" />
                          <div className="p-3">
                            <h4 className="font-medium">{item.productTitle}</h4>
                            <p className="text-sm text-gray-600 mt-1">{item.productDescription}</p>
                          </div>
                ...
                <div className="mt-4 flex gap-3 justify-center">
                  {!item.recreationFailed && (
                    <button onClick={() => onApproveItem([item], 'approve_recreated')} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors">
                      ✓ Add to Wardrobe
                    </button>
                  )}
          ...
          <div className="mt-6 text-center">
            <button onClick={() => onApproveItem(recreatedItems.filter(item => !item.recreationFailed), 'approve_all_recreated')} className="bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition-colors">
              Add All Successfully Recreated Items
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <ItemRecreationWorkflow
        ...
        originalImage={recreationOriginalImage}
        onApproveItem={handleApproveRecreatedItems}
        onResetRecreation={() => setShowRecreationWorkflow(false)}
      />
    )}
    </div>
