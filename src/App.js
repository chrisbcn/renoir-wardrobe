{wardrobe.length === 0 && uploadingItems.length === 0 ? (
  <div className="empty-state">
    <p className="empty-state-text">No items in wardrobe yet</p>
    <p className="empty-state-subtext">Upload clothing photos for detailed luxury analysis</p>
  </div>
) : (
  <>
    <div style={{
      ...
      {uploadingItems.map(item => (
        <div key={item.id} style={{ cursor: 'pointer', position: 'relative' }}>
          <div style={{ position: 'relative', border: '1px solid #E0E0E0', aspectRatio: '3/4' }}>
            <img 
              src={item.imageUrl} 
              alt={item.name}
              style={{ 
      ...
      {wardrobe.map(item => (
        ...
        >
          ...
          {item// App.js - Complete version with pagination and detailed analysis
...
>
  ...
  >
    <div className="modal-header">
      <h2 style={{ fontSize: '20px', fontWeight: '600' }}>{selectedItem.name}</h2>
      <button 
        onClick={() => setSelectedItem(null)}
    ...
    <div className="modal-body">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '32px' }}>
        <img 
          ...
          style={{ width: '100%', height: 'auto', border: '1px solid #E0E0E0', position: 'sticky', top: 0 }}
        />
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {selectedItem.analysis?.error ? (
            <p style={{ color: '#EF4444' }}>Analysis failed: {selectedItem.analysis.error}</p>
          ) : (
            <>
              {/* Overall Assessment */}
              {selectedItem.analysis?.overallAssessment && (
                <div className="analysis-card" style={{ background: '#F8F4FE' }}>
                  <h3>Overall Assessment</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '14px' }}>
                    <p><span style={{ fontWeight: '500' }}>Tier:</span> {selectedItem.analysis.overallAssessment.tier}</p>
                    <p><span style={{ fontWeight: '500' }}>Est. Retail:</span> {selectedItem.analysis.overallAssessment.estimatedRetail}</p>
                    <p><span style={{ fontWeight: '500' }}>Condition:</span> {selectedItem.analysis.overallAssessment.condition}</p>
                    <p><span style={{ fontWeight: '500' }}>Age:</span> {selectedItem.analysis.overallAssessment.estimatedAge}</p>
                    <p style={{ gridColumn: 'span 2' }}><span style={{ fontWeight: '500' }}>Authenticity:</span> {selectedItem.analysis.overallAssessment.authenticityConfidence}</p>
                  </div>
                </div>
              )}
